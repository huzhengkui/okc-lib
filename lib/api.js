...
var Api = function Api(provider) {
  ...
  var sendTransaction = send("okc_sendTransaction");
  var sendRawTransaction = send("okc_sendRawTransaction");
  var getTransactionReceipt = send("okc_getTransactionReceipt");
  var compileSolidity = send("okc_compileSolidity");
  var call = send("okc_call");
  var getBalance = send("okc_getBalance");
  var accounts = send("okc_accounts");
  ...
  var addTransactionDefaults = async function addTransactionDefaults(tx) {
    var baseDefaults = [tx.chainId || send("net_version")(), tx.gasPrice || send("okc_gasPrice")(), tx.nonce || send("okc_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"];
    ...
    var gas = tx.gas ? tx.gas : Nat.div(Nat.mul((await send("okc_estimateGas")({
      from: tx.from,
      to: tx.to,
      value: tx.value,
      nonce: tx.nonce,
